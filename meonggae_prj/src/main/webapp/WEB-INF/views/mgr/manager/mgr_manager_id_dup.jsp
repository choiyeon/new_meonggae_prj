<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	info="관리자 - 관리자 관리 - 관리자 등록 - 아이디 중복 확인"%>
...
<script type="text/javascript">
	$(function() {
		...
		$("#btn").click(function() {
			...
			let flagValidation = idLength(id, minLength, maxLength) && onlyNumberAndEnglish(id);
			if(flagValidation) {
				if(callAjaxCanUseId(id)) {
					if(confirm('이 아이디는 사용 가능한 아이디입니다. 사용하시겠습니까?')) {
						$(opener.document).find('#managerId').val($("#id").val());
						self.close();
					} // end if
				} else {
				...
				alert('아이디는 8자 이상 15자 이하의 소문자 영문과 숫자로만 구성');
			} // end else
		}); // click
	}); // $(document).ready(function() { })
	
	// ajax - 관리자 아이디 중복 확인
	function callAjaxCanUseId(id) {
		var flagCanUse = false;
		var param = {
				managerId: id
		}
		$.ajax({
			url: "http://localhost${pageContext.request.contextPath}/mgr/manager/mgr_manager_id_dup_chk.do",
			type: "GET",
			data: param,
			async: false, 
			dataType: "JSON",
...
<div class="nk-block-head nk-block-head-sm">
	<div class="nk-block-between">
		<div class="nk-block-head-content">
			<h3 class="nk-block-title page-title">관리자 아이디 중복 확인</h3>
			<div class="nk-block-des text-soft"><p>아이디는 8자 이상 15자 이하의 소문자 영문과 숫자로만 구성</p></div>
		</div>
	</div>
</div>
